...
How this project's documentation is managed — **only the documents the current built-in graphs actually consume are listed**; everything else in `docs/` is legacy, kept for reference, not consumed by any graph. The graph runtime delivers context through channels: the convention layer (platform-default-loaded), project-layer config globs, constraints, and run state. What the 10 built-in graphs actually consume:
...
|Canonical-description record (`docs/adr/non-adr-canonical-description-english-only.md`)|Canonical description English-only policy (bilingual READMEs; description never translated)|

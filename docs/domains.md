...
|`engine-feature`|Engine feature-point domain|37|
...
|graph-fidelity|engine-feature|Engine-feature domain — platform-seam signal discipline module (ADR 0151/0152 + R2 suspension ADR 0175 + module split ADR 0176): echo-only chain (identity + progress) in pure data core (`core/` chain/echo-line/runframe/pcl/shapes; reduction/metering/settlement machinery suspended — reference tree relocated to graph-fidelity-context); interface encapsulation layer (`src/interfaces/` SignalLifecycle + DisplayFeedback, effect Schema contracts, `exports["./interfaces"]` type-only); dual-face adapters typed against real platform contracts (OMP `ExtensionAPI` factory + opencode `{ server: Plugin }`), one parametric shape seam (`core/shapes.ts`); native distribution (`omp.extensions` manifest + `exports["./server"]` + marketplace catalog entry; no generated deploy copy, ADR 0153); observability = PCL mark only (appendEntry); subagent coverage platform-verified; supersedes signal-seams + context-fidelity|active|core|
|graph-fidelity-context|engine-feature|Engine-feature domain — R2 cost-economy reference package (ADR 0176 module split): suspended context-management tree (classify/reduce/compress/state/ledger/settle/headroom-client etc.) + vendored core copies, private: true, reference-only redesign input (ADR 0175 C3); loading = manifest second entry (disabled = unregistered), docking = type-only interface import + platform channels|active|core|
...
67 domains total (19 skill — 17 active + 2 retired — + 10 graph + 38 engine-feature — 30 active + 8 retired). Status: 57 active + 10 retired.
...
|context-usage-contract|engine-feature|ADR 0175 + change archive (graph-fidelity-r2-suspend)|RETIRED — R2 cost economy suspended pending redesign (ADR 0175); spec removed at change archive; skill sections retained as reference (HLT-REGISTRY §compress/§headroom disabled, atom-phase-handler Checks context row estimate-only, atom-pilot DISPLAY Context/Tools stats Checks-rows-only)|
...
|graph-fidelity|Platform-seam signal discipline (ADR 0151/0152 + module split ADR 0176) — base module + interface layer: `core/` pure data layer (types/chain/echo-line/runframe/pcl/resident-data, zero platform imports) + `src/interfaces/` (SignalLifecycle assembly/injection/echo/restore + DisplayFeedback audit; effect Schema contracts; `exports["./interfaces"]` type-only) + `adapters/` typed against real platform contracts (OMP `ExtensionAPI` factory from `@oh-my-pi/pi-coding-agent`; opencode `{ server: Plugin }` from `@opencode-ai/plugin`); distribution = `omp.extensions` manifest + `exports["./server"]` + marketplace catalog entry (registration surface; no generated deploy copy, ADR 0153); no rules/ asset (TTSR rule file deleted, ADR 0154)|`packages/graph-fidelity/` (src/ core + interfaces + adapters + diagnostics, test/)|`openspec/specs/graph-fidelity/spec.md`|signal-distribution|core|ro platform imports) + `src/interfaces/` (SignalLifecycle assembly/injection/echo/restore + DisplayFeedback audit; effect Schema contracts; `exports["./interfaces"]` type-only) + `adapters/` typed against real platform contracts (OMP `ExtensionAPI` factory from `@oh-my-pi/pi-coding-agent`; opencode `{ server: Plugin }` from `@opencode-ai/plugin`); distribution = `omp.extensions` manifest + `exports["./server"]` + marketplace catalog entry (registration surface; no generated deploy copy, ADR 0153); no rules/ asset (TTSR rule file deleted, ADR 0154); observability = PCL mark only (appendEntry — opencode absence declared); subagent coverage platform-verified; R2 cost economy suspended (ADR 0175) — reference tree relocated to graph-fidelity-context (ADR 0176); single frame lookup (role-order parameterization), single failure-diagnostics point (adapters/diagnostics.ts); supersedes signal-seams + context-fidelity|`packages/graph-fidelity/` (src/ core + interfaces + adapters + diagnostics, test/)|`openspec/specs/graph-fidelity/sp…|
|graph-fidelity-context|R2 cost-economy reference package (ADR 0176 module split): suspended context-management tree relocated from graph-fidelity (classify/reduce/compress/state/ledger/settle/echo-benefit/headroom-client etc.) + vendored core copies (runframe/discipline/types — ADR 0175 C3); private: true, never published, never deployed, reference-only redesign input; zero runtime imports; loading = manifest second entry (disabled = unregistered — deployment surfaces stay single-entry), docking = type-only interface import (exports["./interfaces"]) + platform channels (OMP EventBus/appendEntry, opencode client); bare runtime imports forbidden (ADR 0166); reference tests green + docking test|`packages/graph-fidelity-context/` (src/ context-management + core vendored, test/)|`openspec/specs/graph-fidelity/module-split/spec.md`|graph-fidelity||core|
...
|`packages/graph-fidelity-context/` (src/ context-management + vendored core, test/)|graph-fidelity-context|

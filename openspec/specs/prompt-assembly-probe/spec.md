...
MODIFIED: probe group E SHALL assert both faces per the native tier: the opencode face (graph-fidelity package) SHALL contain the fidelity primitives (dedup keep-latest, errored-result reduction marker, structural protection set); the OMP face SHALL assert the native mechanisms — the `context` event exists in the npm dist, the `compaction.supersedeReads` setting exists with default-on semantics, and the `elideUneventfulResults` setting exists. No "OMP face absent" assertion exists.

#### Scenario: opencode face present

- **WHEN** group E scans `packages/graph-fidelity/src/core/transform.ts`
- **THEN** it finds `buildFidelityPlan` / `applySessionFidelity` / `ERROR_MARKER` / the `[input removed due to failed tool call]` marker

#### Scenario: OMP native tier asserted

- **WHEN** group E scans the npm `pi-coding-agent` dist and settings schema
- **THEN** it finds the `context` event emission path, `supersedeReads` (default on), and `elideUneventfulResults`, and records them as the native tier in the outputs JSON
...
The full six-group suite SHALL run in CI via `bun test` with npm-installed dependencies; no platform install, no local OMP checkout, no Rust build is required.

#### Scenario: clean checkout run

- **WHEN** the suite runs on a clean checkout with `bun install`

...
describe('atom-phase-handler SKILL.md - project constraints + gate branch contract', () => {
  ...
  it('documents NO constraints/runMode NodeDetail fields - prologue outputs instead', () => {
    ...
    expect(skill).toMatch(/Prologue Output Contract \/ §Prologue Context Blocks \(single home\)/);
    expect(skill).toMatch(/CONTEXT-ASSEMBLY\.md §Prologue Output Contract/);
  });
  ...
  it('documents the prologue degradation rule - missing output never blocks', () => {
    expect(skill).toMatch(
      /Source, paths, presence gating, degrade: see CONTEXT-ASSEMBLY\.md §Prologue Output Contract/,
    );
  ...
  it('defines the §Tool Usage Check scenario-keyed section', () => {
    expect(skill).toMatch(/## Tool Usage Check - scenario-keyed/);
    const section = skill.slice(skill.indexOf('## Tool Usage Check'));
    expect(section).toMatch(/Tool usage check:/);
    expect(section).toMatch(/used: locate \(in-project code\) — <chain-head evidence/);
    expect(section).toMatch(/violated: write \(in-project code\)/);
  ...
  it('generates the violation marker mechanically - never self-issued, missing block = all-class violation', () => {
    ...
    expect(section).toMatch(/no check block -> all declared scenarios violated/);
  });

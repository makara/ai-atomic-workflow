...
describe('F8 · echo format pins (R1 identity-only)', () => {
  it('no mode segment ever rides the echo — the mode knob is gone (R2 style prompts removed)', async () => {
  ...
  it('echo carries the progress segment when the frame carries N/M', async () => {
    const { api, handlers } = stubApi();
    ompExtension(api);
    const context = handlers.get('context') as (e: never) => unknown;
    const result = (await context({
...
describe('F8 · context event guards', () => {
  it('malformed events (wrong type, non-array) degrade to undefined', async () => {
    const { api, handlers } = stubApi();
    ompExtension(api);
    const handler = handlers.get('context') as (e: never) => unknown;
    expect(await handler({ type: 'not-context', messages: [] } as never)).toBeUndefined();
    expect(await handler({ type: 'context', messages: 'not-an-array' } as never)).toBeUndefined();
    expect(await handler(undefined as never)).toBeUndefined();
  });

  it('no anchored frame → no echo (identity source rule)', async () => {
    const { api, handlers } = stubApi();
    ompExtension(api);
    const context = handlers.get('context') as (e: never) => unknown;
    const result = await context({

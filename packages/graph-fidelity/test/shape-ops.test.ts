...
import { normalizeOneToEchoMessage, normalizeToEchoMessages } from '../src/core/shapes.js';
...
describe('single-message normalize (op 8)', () => {
  it('content-less payload normalizes to undefined (no-op)', () => {
    expect(normalizeOneToEchoMessage(asOmp({}), OMP_SHAPE)).toBeUndefined();
  });

  it('duck-typed user-role variant keeps block-level shapes', () => {
    const echo = normalizeOneToEchoMessage(
      asOmp({ role: 'user', content: [{ type: 'tool-result', toolCallId: 't2', content: 'block result' }] }),
      OMP_SHAPE,
    );
    expect(echo?.isToolResult).toBe(true);
    expect(echo?.toolResultIds).toEqual(['t2']);
    expect(echo?.text).toBe('block result');
  });

  it('opencode container normalizes through the same single-message form', () => {
    const echo = normalizeOneToEchoMessage(
      asOc({ role: 'user', parts: [{ type: 'tool', toolCallId: 't1', content: 'x' }] }),
      OPENCODE_SHAPE,
    );
    expect(echo?.isToolResult).toBe(true);
    expect(echo?.text).toBe('x');
    expect(echo?.toolResultIds).toEqual(['t1']);
  });

 ...
 * (`core/echo-line.ts` `renderIdentityEcho`). The R2 benefit-graphic
 * renderer pins moved with the suspended machinery to
 * `test/context-management/echo-benefit.test.ts` (ADR 0175).
 */
...
describe('renderIdentityEcho', () => {
  ...
  it('omits the progress segment when the frame carries none', () => {
    expect(renderIdentityEcho({ nodeId: 'a/b', progress: undefined })).toBe('▣ [seam] node a/b');

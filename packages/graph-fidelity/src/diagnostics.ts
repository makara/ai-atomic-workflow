/**
 * Failure diagnostics — the single process-log line on adapter failure
 * paths (ADR 0176 F6). The zero-deny try/catch stays per site; the
 * diagnostic line is single-sourced here. One line per failure:
...
/** Report one failure line (site label + error message). */
